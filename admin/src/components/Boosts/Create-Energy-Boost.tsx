...
const CreateEnergyBooster: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    submissionData.append("energy", String(formData.energy));

    dispatch(createBoosts({ type: "energy", formData: submissionData }))
      .unwrap() // Unwrap the action to handle promise rejections
      .then((newBooster: Boost) => {
        // Success
        dispatch(addBooster({ type: "energy", booster: newBooster })); // Pass type and booster
        ...
        <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
          <div className="border-b border-stroke px-7 py-4 dark:border-strokedark">
            <h3 className="font-medium text-black dark:text-white">
              Create Energy Booster
            </h3>
          </div>

          {showSuccessPop && (
            <SuccessPop
              title="Success! New Booster Created!"
              onClose={handleClose}
            />
          )}
...
export default CreateEnergyBooster;
