...
const EnergyBoostTable = () => {
    const dispatch = useAppDispatch();
    const { energyBoosts, pagination, isLoading, error } = useAppSelector((state) => state.booster);
    ...
    useEffect(() => {
        dispatch(fetchBoosts({ type: "energy", page, limit }));
    }, [page, dispatch]);
    ...
    const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const filteredBoosts = energyBoosts?.filter(boost =>
        ...
        <div className="flex-1 rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1 mt-5">
            <div className="flex justify-between items-center mb-6">
                <h3 className="font-medium text-black dark:text-white">
                    Energy Boost List ({pagination?.totalBoosts})
                </h3>
                <input
                    type="text"
                    ...
                    <tbody className="text-sm">
                        {filteredBoosts?.map((boost) => (
                            <tr key={boost._id}>
                                ...
                                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                                    <p className="text-black dark:text-white font-medium text-xs">{boost.energy}</p>
                                </td>
...
export default EnergyBoostTable;
